...
description: "A playful, voice-interactive detective game built with Next.js. Designed for accessibility and performance, with an engaging narrative that keeps players curious from start to finish."
demo: "whoatethecookie.fun"
---

![Project screenshot](/assets/who-ate-the-cookie.webp)
...
<li>Voice warm-up strategy to eliminate first-click playback delays.</li>
...
<li>**Audio playback:** Responses prepared with headers for blob consumption; session storage caching with `useEffect()` + `setTimeout()` to pre-run voices and avoid API rate limits.</li>
...
<ul className="list">
<li>**Autoplay restrictions:** Chrome blocked audio without user interaction. Fixed with explicit gesture checks and an audio warm-up strategy.</li>
<li>**Cross-browser playback:** Audio behaved inconsistently across browsers. Improved handling with `audioRef` refinements and fallback testing.</li>
...
<li>**Accessibility testing:** Verified usability with keyboard-only navigation and `macOS VoiceOve`r, resulting in a more inclusive experience.</li>
</ul>
