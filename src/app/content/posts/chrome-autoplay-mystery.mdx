...
Although I could see the properties, I couldn’t override them directly — the audio element’s behavior still didn’t budge. <span className="emoji-small">🤔</span>
...
**Valuable lesson learned:** Only to realize — I forgot `.then()` after `.play()` <span className="emoji-small">🙃</span>, and maybe someone else will `.catch()` that. <span className="emoji-small">😏</span>
...
**Valuable lesson learned (again):** Don't try to think through too many things too speedily at once. <span className="emoji-small">🤣</span> Slow down, focus, take it step by step, and only one step at a time!